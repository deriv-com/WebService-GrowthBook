...
subtest "boolean" => sub {
 my $test_cases = [[1,1,1,0],[0,0,0,1]];
    for my $case ($test_cases->@*){
        my $result = WebService::GrowthBook::FeatureResult->new(id => 'test', value => $case->[0] ? JSON::MaybeXS::true : JSON::MaybeXS::false);
...
subtest "number" => sub {
    ...
    for my $case ($test_cases->@*){
        my $result = WebService::GrowthBook::FeatureResult->new(id => 'test', value => $case->[0]);
        is($result->value, $case->[1], "$case->[0] value is $case->[1]");
    }
};
...
subtest "call on/off on non-boolean" => sub {
    my $result = WebService::GrowthBook::FeatureResult->new(id => 'test', value => 1);
    $log->clear;
